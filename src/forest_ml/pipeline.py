...
def create_pipeline(
    ...
    logreg_c: float, n_estimators: int, max_depth: int, hyperparameter_search: bool,
) -> Pipeline:
    pipeline_steps = []
    ...
    if use_feature_engineering and type_feature_engineering.lower() == 'pca':
        pipeline_steps.append(("feature_engineering", PCA(random_state=random_state, n_components=15)))
    ...
    if hyperparameter_search:
        if type_model.lower() == 'logisticregression':
            search = GridSearchCV(LogisticRegression(random_state=random_state), param_lr, scoring='accuracy', cv=cv_inner, refit=True)
            pipeline_steps.append(("classifier", search))
        elif type_model.lower() == 'randomforestclassifier':
            search = GridSearchCV(RandomForestClassifier(random_state=random_state), param_rf, scoring='accuracy', cv=cv_inner, refit=True)
            pipeline_steps.append(("classifier", search))
        else:
